...
export function ErrorState({ title = 'Something went wrong', message = 'Please try again later.', onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      ...
      <h3 className="text-lg font-bold text-dark-800 mb-1">{title}</h3>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      <div className="w-16 h-16 bg-sand-100 rounded-full flex items-center justify-center mb-5">
        {icon || <Inbox className="w-7 h-7 text-dark-300" />}
      </div>
      <h3 className="text-lg font-bold text-dark-800 mb-1">{title}</h3>
